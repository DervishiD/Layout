...
class Vector {
     ...
     */
    infix fun angle(v : Vector) : Double = acos(abs((this * v) / (norm() * v.norm())))

    /**
     * Stretches this Vector to the given length
     */
    infix fun toLength(length : Double){
        unit()
        x *= length
        y *= length
    }
     ...
     * Dot product.
     */
    infix fun dot(v : Vector) : Double = this * v

    /**
     * Norm of the cross product aka determinant
     */
    infix fun cross(v : Vector) : Double = x * v.y - y * v.x
     ...
     * */
    operator fun times(v : Vector) : Double = x * v.x + y * v.y
     ...
     */
    operator fun times(d : Double) : Vector = Vector(d * x, d * y)
     ...
     * @return Scaled Vector.
     */
    operator fun times(d : Int) : Vector = Vector(d * x, d * y)
     ...
     */
    operator fun div(d : Int) : Vector = this / d.toDouble()
     ...
     * Rotation
     */
    operator fun rem(d : Double) = rotate(d)
